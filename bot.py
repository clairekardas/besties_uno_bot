...
from results import (add_call_bluff, add_choose_color, add_draw, add_gameinfo,
                     ...
                     add_card, add_mode_classic, add_mode_fast, add_mode_wild, add_mode_rainbow, add_mode_text)
...
def close_game(update: Update, context: CallbackContext):
    ...
    if user.id in game.owner:
        game.open = False
        send_async(context.bot, chat.id, text=_("Closed the game lobby 😔"
...
def process_result(update: Update, context: CallbackContext):
    ...
    if result_id in ('hand', 'gameinfo', 'nogame'):
        ...
        send_async(context.bot, chat.id, text=__("GChanged the gamemode to {mode} 🤩".format(mode = mode)))
        return
